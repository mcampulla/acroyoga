...
namespace acroyoga.it.admin.Controllers
{
    ...
    public class EventsController : ODataController
    {
        ...
        [EnableQuery]
        public IHttpActionResult GetEvent([FromODataUri]int key)
        {
            var model = _ctx.Events.FirstOrDefault(p => p.EventId == key);
        ...
        //    var current = _ctx.Events.FirstOrDefault(p => p.EventId == key);
        //    if (current == null)
        //    {
        //        return NotFound();
        //    }

        //    model.EventId = current.EventId;
